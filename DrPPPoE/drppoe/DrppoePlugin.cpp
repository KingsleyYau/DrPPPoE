	...
	void onRun() {
		if(m_pDrppoePlugin) {
			...
			while(m_pDrppoePlugin->IsRunning()) {
					...
					if(DRCOM_ERR != m_pDrppoePlugin->DrcomDialExtProtoHandle_ChallengeRep(pBuffer, iLen)) {
						...
						if(m_pDrppoePlugin->DrcomDialExtProtoSendLoginPacket()) {
							...
							if(DRCOM_ERR != m_pDrppoePlugin->DrcomDialExtProtoHandle_LoginRes(pBuffer, iLen)) {
								...
								m_pDrppoePlugin->LockFirstTimeAnti();
								if(m_pDrppoePlugin->GetIsFirstTimeAnti()) {
									// 第一次防代理认证，需要发三个包
									iAntiTimes = 3;
									m_pDrppoePlugin->SetFirstTimeAnti(false);
									m_pDrppoePlugin->UnLockFirstTimeAnti();
								}
								else {
									// 其他只发2个包
									iAntiTimes = 2;
								...
								for(int i = 0; i < iAntiTimes; i++) {
									if(DRCOM_ERR != m_pDrppoePlugin->SendNextDownloadModuleFileCmd()) {
										...
										ILog("jni.DrppoePlugin::HandleClientRecvPacket()", "防代理响应处理完成");
									}
								}
							}
						}
				...
				m_pDrppoePlugin->WaitForAuthThread(16);
			}
...
DrppoePlugin::DrppoePlugin() {
	...
	m_pAuthDrThread = new DrThread(new DrSendChallengeRunnable(this));
	m_bIsServiceRunning = false;
	m_pIsFirstTimeAntiMutex = new DrMutex();
	m_pDrSendCond = new DrCond();
}

DrppoePlugin::~DrppoePlugin() {
	// TODO Auto-generated destructor stub
	StopDrPPPoEPlugin();
	delete m_pIsFirstTimeAntiMutex;
	delete m_pDrSendCond;
}

bool DrppoePlugin::StartDrPPPoEPlugin() {
	ILog("jni.DrppoePlugin.StartDrPPPoEPlugin", "开始Dr.COM插件协议");
	if(!ResetParam()) {
		ELog("jni.DrppoePlugin.StartDrPPPoEPlugin", "初始化参数失败!");
		return false;
	}

	if(!ModifyRoute()) {
		ELog("jni.DrppoePlugin.StartDrPPPoEPlugin", "修改默认路由失败!");
		return false;
	}

	if(!m_UdpSocket.Bind(m_iLocalPort)) {
		ELog("jni.DrppoePlugin.StartDrPPPoEPlugin", "绑定端口(%d)失败!", m_iLocalPort);
		return false;
	}
	else {
		ILog("jni.DrppoePlugin.StartDrPPPoEPlugin", "Dr.COM插件绑定端口(%d)成功!", m_iLocalPort);
	}
	...
	// 创建发送线程
	pthread_t pThreadSend = m_pAuthDrThread->start();
	if(pThreadSend <= 0) {
		ELog("jni.DrppoePlugin.StartDrPPPoEPlugin", "创建发送线程失败!");
		m_bIsRunning = false;
		return false;
	}
...
bool DrppoePlugin::StartDrPPPoEPluginService() {
	if(IsServiceRunning()) {
		ILog("jni.DrppoePlugin.StartDrPPPoEPlugin", "Dr.COM插件协议正在运行");
		return true;
	}
	else {
		// 假设认证插件已经运行
		m_bIsServiceRunning = true;
		if(!StartDrPPPoEPlugin()) {
			// 假设认证插件已经运行失败
			m_bIsServiceRunning = false;
		}
	}
	return m_bIsServiceRunning;
}
void DrppoePlugin::StopDrPPPoEPlugin() {
	...
	// 激活认证线程
	m_pDrSendCond->lock();
	m_pDrSendCond->signal();
	m_pDrSendCond->unlock();

	// 退出认证线程
	if(m_pAuthDrThread) {
		m_pAuthDrThread->stop();
	}
	...
	ILog("jni.DrppoePlugin.StopDrPPPoEPlugin", "停止Dr.COM插件协议成功！");
	m_bIsServiceRunning = false;
}
...
bool DrppoePlugin::IsServiceRunning() {
	return m_bIsServiceRunning;
}
DrUdpSocket DrppoePlugin::GetUdpSocket() {
	return m_UdpSocket;
}
void DrppoePlugin::WaitForAuthThread(int second) {
//	int i = 0;
//	// 每秒判断一次是否需要退出,不锁可能会多跑一次
//	while(IsRunning() && i++ < second) {
//		sleep(1);
//	}

	m_pDrSendCond->lock();
	m_pDrSendCond->timedwait(second);
	m_pDrSendCond->unlock();
}
void DrppoePlugin::LockFirstTimeAnti() {
	m_pIsFirstTimeAntiMutex->lock();
}
void DrppoePlugin::UnLockFirstTimeAnti() {
	m_pIsFirstTimeAntiMutex->unlock();
}
...
bool DrppoePlugin::ResetParam() {
	ILog("jni.DrppoePlugin.ResetParam", "初始化参数...");
	m_SetpCurrent = SetpNone;
	m_bIsRunning = false;

	m_iServerPort = 61440;
	m_sServerAddress = "1.1.1.1";
...
//	}
	...
	for(list<IpAddressNetworkInfo>::iterator itr = broadList.begin(); itr != broadList.end(); itr++) {
		...
		int iSrc, iDst, iMsk;
		struct in_addr addr;
		inet_aton(device.netmask.c_str(), &addr);
		iMsk = addr.s_addr;
		...
		if((iSrc & iMsk) == (iDst & iMsk)) {
			...
			ILog("jni.DrppoePlugin::GetPPPoEServerAddress", "获取到PPPoE服务器:%s", broadAddress.c_str());
			break;
		}
	}
...
//    bool bFlag = true;
	...
	{
		ILog("jni.DrppoePlugin::GetCrcCheckField", "nChallengeSize < sizeof(unsigned long)");
		return false;
	}
    ...
    if (type > 0) {
        ...
        if (ENCRYPTION_MODE_NONE == nEncryptionType) {
        	ILog("jni.DrppoePlugin::GetCrcCheckField", "加密方式为无:(%d)", nEncryptionType);
            pCrcBuff[0] = DRCOM_DIAL_EXT_PROTO_CRC_INIT;
            pCrcBuff[1] = 126;
        }
        else if (ENCRYPTION_MODE_MD5 == nEncryptionType) {
        	ILog("jni.DrppoePlugin::GetCrcCheckField", "加密方式为MD5:(%d)", nEncryptionType);
            // 用MD5生成校验码
        	...
        	ILog("jni.DrppoePlugin::GetCrcCheckField", "加密方式为MD4:(%d)", nEncryptionType);
//            // 用MD4生成校验码
//            unsigned char* pCheckBuff = (unsigned char*)pCrcBuff;
//            // MD5 Challenge
//            unsigned char ucEnBuf[16] = {0};
...
//            pCheckBuff[7] = ucEnBuf[12];
        	...
        	ILog("jni.DrppoePlugin::GetCrcCheckField", "加密方式为SHA1:(%d)", nEncryptionType);
            // 用SHA1生成校验码
            unsigned char* pCheckBuff = (unsigned char*)pCrcBuff;
            // MD5 Challenge
		    ...
		    // ERROR 未知类型不能处理
			ILog("jni.DrppoePlugin::GetCrcCheckField", "ERROR 未知类型不能处理");
			return false;
		}
        return true;
    }
...
//		if(string::npos != device.name.find("ppp", 0)) {
//			// found device like 'ppp0'
//			ILog("jni.DrppoePlugin::ModifyRoute", "找到PPPoE拨号接口:(%s)，开始修改默认路由...", device.name.c_str());
//			ResetRoute();
//			string cmd = "ip route add default dev ";
//			cmd += device.name;
//			string result = SystemComandExecuteWithRootWithResult(cmd);
//			if(result.length() == 0) {
//				ILog("jni.DrppoePlugin::ModifyRoute", "修改默认路由成功!");
//				return true;
//			}
//			else {
//				// 添加默认路由失败
//				ELog("jni.DrppoePlugin::ModifyRoute", "修改默认路由失败!");
//			}
//		}
//	}
...
bool DrppoePlugin::ResetRoute() {
	bool bFlag = false;
	string result = SystemComandExecuteWithRootWithResult("ip route del default");
	if(result.length() == 0) {
		ILog("jni.DrppoePlugin::ModifyRoute", "删除默认路由成功!");
		return true;
	}
	return bFlag;
}
...
int DrppoePlugin::SendAuthCmd(char* cmd, int iLen) {
    ...
    ILog("jni.DrppoePlugin::SendAuthCmd", "向%s(%d)发送命令...", m_sServerAddress.c_str(), m_iServerPort);

    int iRet = m_UdpSocket.SendData(m_sServerAddress, m_iServerPort, cmd, iLen);
    if(iRet < 0) {
    	ILog("jni.DrppoePlugin::SendAuthCmd", "网络连接中断，发送认证数据错误!");
    	return DRCOM_ERR;
    }
...
int DrppoePlugin::RecvAuthData(char* pBuffer, unsigned int iLen) {
	...
	if(m_sServerAddress == "1.1.1.1") {
		ILog("jni.DrppoePlugin::RecvAuthData", "发现PPPoE服务器地址:%s!", ip.c_str());
		m_sServerAddress = ip;
	}
...
int DrppoePlugin::DrcomDialExtProtoSendChallenge() {
	...
	ILog("jni.DrppoePlugin::DrcomDialExtProtoSendChallenge", "第1步,发起认证请求...");
...
int DrppoePlugin::DrcomDialExtProtoHandle_ChallengeRep(char *pBuffer, int iLen) {
	ILog("jni.DrppoePlugin::DrcomDialExtProtoSendChallenge", "第2步,接收到服务器认证请求返回...");
	...
	if (iLen >= sizeof(struct _tagDrcomDialExtProtoReturnChallengeAndIP)) {
		if (retchall->hcode == AUTH_CODE_PPPOE && retchall->htype == AUTH_CODE_TYPE_PPPOE_RET_CHALL) {
            ...
            ILog("jni.DrppoePlugin::DrcomDialExtProtoSendChallenge", "第2步,接收到服务器认证请求返回处理完成!");
            return DRCOM_SUC;
		}
	}
	ILog("jni.DrppoePlugin::DrcomDialExtProtoHandle_ChallengeRep", "第2步,认证服务器认证请求的响应处理失败!");
	// 回调错误
	return DRCOM_ERR;
}
...
int DrppoePlugin::DrcomDialExtProtoSendLoginPacket() {
	ILog("jni.DrppoePlugin::DrcomDialExtProtoSendChallenge", "第3步,发送认证数据命令...");
...
//        ILog("jni.DrppoePlugin::DrcomDialExtProtoSendChallenge", "pNewWorkInfo1:%p", pNewWorkInfo);
...
//            	temp = temp + 6;
//            	showLog("jni.DrppoePlugin::DrcomDialExtProtoSendChallenge", "pNewWorkInfo2:%p temp:%p", pNewWorkInfo, temp);
//            }
//        }
//
//    }
...
int DrppoePlugin::DrcomDialExtProtoHandle_LoginRes(char *pBuffer, int iLen) {
	...
	ILog("jni.DrppoePlugin::DrcomDialExtProtoHandle_LoginRes", "第4步,接收到服务器认证数据返回...");
    ...
    if (iLen >= sizeof(struct _tagDrcomDialExtProtoLoginResult)
        ...
        if ((loginRes->htype & 0x40) != 0)
        {
            ...
            {
            	// 系统消息
            	ILog("jni.DrppoePlugin::DrcomDialExtProtoHandle_ChallengeRep", "第4步,接收认证结果处理完成!(系统消息)");
            }
            else
            {
            	// 客户端需要升级，请下载最新的安装包重新安装客户端!
            	ILog("jni.DrppoePlugin::DrcomDialExtProtoHandle_ChallengeRep", "第4步,接收认证结果处理完成!(客户端需要升级，请下载最新的安装包重新安装客户端)");
            }
            ...
            {
            	// url 系统链接
            	ILog("jni.DrppoePlugin::DrcomDialExtProtoHandle_ChallengeRep", "第4步,接收认证结果处理完成!(url 系统链接)");
            }
            return DRCOM_SUC;
        }
        ...
        if ((loginRes->htype & 0x80) != 0)
        {
            ...
            ILog("jni.DrppoePlugin::DrcomDialExtProtoHandle_ChallengeRep", "第4步,接收认证结果处理完成!");
            return DRCOM_SUC;
        }
    ...
    ILog("jni.DrppoePlugin::DrcomDialExtProtoHandle_ChallengeRep", "第4步,接收认证结果处理失败");
	return DRCOM_ERR;
}
...
int DrppoePlugin::DrcomDialExtProtoHandle_ping(char* AuthData, int len) {
	ILog("jni.DrppoePlugin::DrcomDialExtProtoSendChallenge", "发送心跳检测...");
...
{
	ILog("jni.DrppoePlugin::SendNextDownloadModuleFileCmd", "准备发送防代理请求...");
    ...
    {
    	ILog("jni.DrppoePlugin::SendNextDownloadModuleFileCmd", "m_DownloadModuleBuff.DownloadingModuleVer != 0");
    ...
    if (nNowAntiProxyModuleUpdateStatus == AntiProxyModuleUpdateStatus_UPDATING_MODULE)
    {
    	ILog("jni.DrppoePlugin::SendNextDownloadModuleFileCmd", "nNowAntiProxyModuleUpdateStatus == AntiProxyModuleUpdateStatus_UPDATING_MODULE");
        ptrDownloadModuleCmd->ActiveVerNo = m_DownloadModuleBuff.DownloadingModuleVer;
    }
...

    {
        ...
        {
        	ILog("jni.DrppoePlugin::SendNextDownloadModuleFileCmd", "ptrDownloadModuleCmd->code1 == CMD_CHECK_UPDATE && \
        			m_nModuleOnlineCount == m_CHECK_MODULE_UPDATE_INTERVAL");
    ...
    ILog("jni.DrppoePlugin::SendNextDownloadModuleFileCmd", "防代理第%d步, 开始发送请求...", ptrDownloadModuleCmd->code1);
...
{
//	ILog("jni.DrppoePlugin::HandleClientRecvPacket()", "接收到防代理响应, 开始处理...");
	if (nRecvLen < sizeof(DR2133REQINFO)) {
		ILog("jni.DrppoePlugin::HandleClientRecvPacket()", "nRecvLen < sizeof(DR2133REQINFO)");
		return;
	}

	ANTIPROXYDLLGETVERINFORET ptrDllVerInfoBuff;
	ptrDllVerInfoBuff.AuthDataBuff = (unsigned char*)m_AntiProxyModuleAuthDataBuff;
	ptrDllVerInfoBuff.AuthDataLen = nRecvLen;
	ptrDllVerInfoBuff.dllretcode = 1;
	ptrDllVerInfoBuff.serverip = DrSocket::StringToIp(m_sServerAddress.c_str());

	if (!CheckIsModulePacketHeader8(&ptrDllVerInfoBuff))
	{

	}

    DR2133REQINFO* ptrRecvReqInfo = (DR2133REQINFO*)RecvData;
	ILog("jni.DrppoePlugin::IsModuleUpdateFirstPacket()", "防代理第%d步, 接收到响应, 开始处理...", ptrRecvReqInfo->code1);
    ...
    {
    	ILog("jni.DrppoePlugin::HandleClientRecvPacket()", "IsDrcomAntiProxyModulePacket");
        return;
    }
    ...
    if (m_AntiProxyModuleUpdateStatus == AntiProxyModuleUpdateStatus_NOT_SET)
    {
        if (ptrRecvReqInfo->code1 == CMD_CHECK_UPDATE_RES)
        {
            ...
            ILog("jni.DrppoePlugin::HandleClientRecvPacket()", "m_AntiProxyModuleUpdateStatus == AntiProxyModuleUpdateStatus_NOT_SET && ptrRecvReqInfo->code1 == CMD_CHECK_UPDATE_RES");
            return;
        }

        if (IsModuleUpdateFirstPacket(ptrRecvReqInfo, nRecvLen, ptrModuleCheckSumHeader))
        {
        	ILog("jni.DrppoePlugin::HandleClientRecvPacket()", "第一次防代理响应处理...");
            if (m_DownloadModuleBuff.Buffer != NULL)
            {
            	ILog("jni.DrppoePlugin::HandleClientRecvPacket()", "m_DownloadModuleBuff.Buffer != NUL");
                free(m_DownloadModuleBuff.Buffer);
            }
        	...
        	ILog("jni.DrppoePlugin::HandleClientRecvPacket()", "m_DownloadModuleBuff.dwBuffLen:%d", m_DownloadModuleBuff.dwBuffLen);
            if(m_DownloadModuleBuff.dwBuffLen > 0) {
            ...
            if (m_DownloadModuleBuff.Buffer)
            {
            	...
            	ILog("jni.DrppoePlugin::HandleClientRecvPacket()", "m_DownloadModuleBuff.Buffer is not NULL");
                ...
                ILog("jni.DrppoePlugin::HandleClientRecvPacket()", "m_DownloadModuleBuff.DownloadingModuleVer is :%d",  m_DownloadModuleBuff.DownloadingModuleVer);
...
//                        LogMessage("Dr.COM Auth Kernel Driver(U%d)\n", DownloadModuleBuff.DownloadingModuleVer);
        ...
        ILog("jni.DrppoePlugin::HandleClientRecvPacket()", "m_AntiProxyModuleUpdateStatus == AntiProxyModuleUpdateStatus_NOT_SET");
        return;
    }
    ...
    if (m_AntiProxyModuleUpdateStatus == AntiProxyModuleUpdateStatus_BEGIN_UPDATE)
    {
        ...
        ILog("jni.DrppoePlugin::HandleClientRecvPacket()", "m_AntiProxyModuleUpdateStatus == AntiProxyModuleUpdateStatus_BEGIN_UPDATE");
        return;
    }
...
{
	ILog("jni.DrppoePlugin::HandleClientRecvPacket_code1eq2()", "开始处理CMD_CHECK_UPDATE_RES");
...
void DrppoePlugin::SetFirstTimeAnti(bool bFlag) {
	m_bIsFirstTimeAnti = bFlag;
}
...
int DrppoePlugin::CheckIsModulePacketHeader8(ANTIPROXYDLLGETVERINFORET* lpClientInParam) {
    int nTmpHeaderLen = 12 + 4;
    int nModuleDataMinLen = nTmpHeaderLen + sizeof(ANTIPROXYMODULECHECKSUMHEAD);
    m_ptrServerDllPacketHead = (ANTIPROXYMODULECHECKSUMHEAD*)&lpClientInParam->AuthDataBuff[nTmpHeaderLen];

    if (lpClientInParam->AuthDataLen < nModuleDataMinLen)
    {
        return false;
    }

    if (lpClientInParam->AuthDataBuff[4] != 2)
    {
        return false;
    }
...
int DrppoePlugin::CheckIsModulePacketHeader5(ANTIPROXYDLLGETVERINFORET* lpClientInParam) {
    int nTmpHeaderLen = 10 + (2 + 2 + 2) + 4 + 4 + 2;
    int nModuleDataMinLen = nTmpHeaderLen + sizeof(ANTIPROXYMODULECHECKSUMHEAD);
    m_ptrServerDllPacketHead = (ANTIPROXYMODULECHECKSUMHEAD*)&lpClientInParam->AuthDataBuff[nTmpHeaderLen];
    ...
    //g_ClientAuthIP = * (ULONG*)(&lpClientInParam->AuthDataBuff[ClientAuthsourceIPOffset]);
    //CheckRouterByAuthIP(g_ClientAuthIP);
    return true;
}
