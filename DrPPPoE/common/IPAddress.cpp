	...
	if ((fd = socket(AF_INET, SOCK_DGRAM, 0)) >= 0) {
		...
		if (!ioctl(fd, SIOCGIFCONF, (char*)&ifc)) {
			...
			while (intrface-- > 0){
				...
				// MAC
//				if (!(ioctl(fd, SIOCSIFLLADDR, (char *) ifr_ip))) {
                    struct sockaddr_dl *sdl = (struct sockaddr_dl *)&ifr_ip->ifr_addr;
                    int a,b,c,d,e,f;

                    strcpy(cBuffer, (char *)ether_ntoa((const struct ether_addr *)LLADDR(sdl)));
                    sscanf(cBuffer, "%x:%x:%x:%x:%x:%x", &a, &b, &c, &d, &e, &f);
                    sprintf(cBuffer, "%02X:%02X:%02X:%02X:%02X:%02X",a,b,c,d,e,f);
					info.mac = cBuffer;
