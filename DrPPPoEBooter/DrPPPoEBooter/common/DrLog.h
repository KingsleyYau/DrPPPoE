...
#define showLog(tag, format, ...) printf(format, ## __VA_ARGS__)
#define DLog(tag, format, ...) FileLog("pppoetest", format, ## __VA_ARGS__)
#define ILog(tag, format, ...) FileLog("pppoetest", format, ## __VA_ARGS__)
#define ELog(tag, format, ...) FileLog("pppoetest", format, ## __VA_ARGS__)
...
static inline int LogToFile(const char *fileNamePre, int level, const char *fmt, ...) {
    ...
    // 生成日志文件目录
    string sLogFileDir = "/private/var/mobile/Media/log";
