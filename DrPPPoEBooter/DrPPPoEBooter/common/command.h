 ...
 * destDirPath:	目标目录
 */
static inline bool RootNonExecutableFile(string sourceFilePath, string destDirPath) {
	bool bFlag = false;
	...
	if(MountSystem()) {
		...
		if(result.length() == 0) {
			// 拷贝成功
			DLog("jni.command::RootExecutableFile", "拷贝成功%s到%s成功!", sourceFilePath.c_str(), fileName.c_str());
			bFlag = true;
		}
		else {
			ELog("jni.command::RootExecutableFile", "拷贝成功%s到%s失败!", sourceFilePath.c_str(), fileName.c_str());
		}
	}
 ...
 * destDirPath:	目标目录
 */
static inline bool RootExecutableFile(string sourceFilePath, string destDirPath = "/system/bin/") {
	bool bFlag = false;
	...
	if(RootNonExecutableFile(sourceFilePath, destDirPath)) {
		fileName = destDirPath + fileName;
		sprintf(pBuffer, "chmod 4777 %s", fileName.c_str());
		result = SystemComandExecuteWithRootWithResult(pBuffer);
		if(result.length() == 0) {
			// 更改权限成功
			DLog("jni.command::RootExecutableFile", "提升%s权限为4777成功!", fileName.c_str());
			bFlag = true;
		}
		else {
			ELog("jni.command::RootExecutableFile", "提升%s权限为4777失败!", fileName.c_str());
		}
	}
